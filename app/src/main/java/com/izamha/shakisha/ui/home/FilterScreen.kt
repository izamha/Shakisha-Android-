...
import com.izamha.shakisha.ui.components.FilterChip
...
fun FilterScreen(
    ...
    var sortState by remember { mutableStateOf(FoundItemRepo.getSortDefault()) }
    var maxCalories by remember { mutableStateOf(0f) }
    val defaultFilter = FoundItemRepo.getSortDefault()

    Dialog(onDismissRequest = onDismiss) {

        val priceFilters = remember { FoundItemRepo.getPriceFilters() }
        val categoryFilters = remember { FoundItemRepo.getCategoryFilters() }
        val lifeStyleFilters = remember { FoundItemRepo.getLifeStyleFilters() }
        JetsnackScaffold(
            topBar = {
                TopAppBar(
                    navigationIcon = {
                    ...
                    backgroundColor = ShakishaTheme.colors.uiBackground
                )
            }
        ) {
            Column(
...
@Composable
fun SortFilters(
    sortFilters: List<Filter> = FoundItemRepo.getSortFilters(),
    sortState: String,
...
fun MaxCalories(sliderPosition: Float, onValueChanged: (Float) -> Unit) {
    ...
    Slider(
        ...
        colors = SliderDefaults.colors(
            thumbColor = ShakishaTheme.colors.brand,
            activeTrackColor = ShakishaTheme.colors.brand
        )
    )
}
